...
% The following is not optimized, but it does the job.
%  Example useage:
%  FlowerDemo
%  A=splitPetals(Petals);
%  imagesc(A.left);
%  imagesc(A.right);
%  imagesc(A.mid);

% May 17:  Got the code finished to put the petal data back in as a rectangular array.  Uses the cluster index as "color".
% May 18:  Added an experimental section to load the original data (in individual pics).  Works as long as algorithm knows the image file.
%
...
% 
% F=imread('P105F4.jpg');
% A.left=zeros(TLxmax-TLxmin+1,TLymax-TLymin+1,3);
% A.right=zeros(TRxmax-TRxmin+1,TRymax-TRymin+1,3);
% A.mid=zeros(TMxmax-TMxmin+1,TMymax-TMymin+1,3);
% 
% [m,n]=size(TL);
% for j=1:m
%     if TL(j,4)~=0
%       xx=TL(j,1)-TLxmin+1; yy=TL(j,2)-TLymin+1;
%       A.left(xx,yy,:)=F(TL(j,2),TL(j,1),:);
%     end
% end
% [m,n]=size(TR);
% for j=1:m
