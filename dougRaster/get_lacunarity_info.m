...
for i = 1:length(pic_files)
  ...
  %   Processing the current sameple_name_file photo:
      ...
      Name{i}=Petals.Name;
      Left(i)=Petals.Lacunarity(1);
      Right(i)=Petals.Lacunarity(2);
      Mid(i)=Petals.Lacunarity(3);
...
ss=join([dirstr,'/',folder_path,'LacunarityData.mat']);
save(ss, 'T');

writetable(T,'LacunarityTable.xls');
